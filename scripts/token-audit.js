...
const RULES = [
  ...
  },
  {
    ...
    pattern: /rgba?\(\s*\d+[\s,]+\d+[\s,]+\d+(?:[\s,/]+[\d.]+%?)?\s*\)/g,
    severity: 'error',
    suggest() {
    ...
    skipIf(line, match, matchIndex) {
      ...
      if (matchIndex > 0 && /var\([^)]*$/.test(line.slice(0, matchIndex))) return true;
      return false;
    },
  },
  {
    ...
    severity: 'error',
    suggest(match) {
      const raw = match.match(/(0?\.\d+)/)?.[1];
      const val = raw && !raw.startsWith('0') ? `0${raw}` : raw;
      return OPACITY_MAP[val] ? `var(${OPACITY_MAP[val]})` : 'define an opacity token';
    },
    skipIf(line, match, matchIndex) {
  ...
  },
  {
    ...
    suggest(match) {
      ...
      return tokens.length === 1 ? `var(${tokens[0]})` : `var(${tokens.join(') or var(')})`;
    },
    skipIf(line, match, matchIndex) {
  ...
  },
  {
    ...
    pattern: /font-size\s*:\s*([\d.]+(?:px|rem|em))(?!\s*!important)/g,
    severity: 'error',
    suggest() {
  ...
  // ---- WARNINGS: Should ideally use tokens ----
  {
    ...
    pattern: /(?:padding|margin|gap)\s*:\s*(?!var\()(?!0(?:\s|;|$))([^;]*\d+px[^;]*)/g,
    severity: 'warning',
    suggest() {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (const rule of RULES) {
      ...
      while ((match = rule.pattern.exec(line)) !== null) {
        ...
        if (rule.name === 'hardcoded-opacity' || rule.name === 'raw-transition-duration') {
...
function formatViolation(v) {
  ...
  const sevLabel = v.severity === 'error' ? '\x1b[31merror\x1b[0m' : '\x1b[33mwarning\x1b[0m';
...
function main() {
  const cssFiles = findCSSFiles(SRC_DIR).filter((f) => !TOKEN_FILES.has(path.resolve(f)));
  ...
  console.log(`\x1b[1mTotal: ${errors.length} errors, ${warnings.length} warnings\x1b[0m`);
